...
- (void)scrollToIndex:(NSInteger)index animated:(BOOL)animated
{
    ...
    if (index == 0 ) {
        [self.scrollView setContentOffset:CGPointMake(-self.contentInset.left, 0) animated:YES];
    }else{
...
{
    if (scrollView.contentOffset.x < -self.contentInset.left/2) {
        [self scrollToIndex:0 animated:YES];
        // [self didScrollToIndex:self.currentIndex];
        return;
    }else if ((scrollView.contentOffset.x >= -self.contentInset.left/2) && (scrollView.contentOffset.x < 0)){
        [self scrollToIndex:1 animated:YES];
       //  [self didScrollToIndex:self.currentIndex];
        return;
    }
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    ...
    if (scrollView.contentOffset.x < -self.contentInset.left/2 && velocity.x==0) {
        //[self scrollToIndex:0 animated:YES];
         [self didScrollToIndex:self.currentIndex];
        return;
    }else if ((scrollView.contentOffset.x >= -self.contentInset.left/2) && (scrollView.contentOffset.x < 0 && velocity.x==0)){
       // [self scrollToIndex:1 animated:YES];
          [self didScrollToIndex:self.currentIndex];
        return;
    }
